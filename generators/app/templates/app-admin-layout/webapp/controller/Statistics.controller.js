sap.ui.define(['<%= projectNamespaceAMD %>/controller/BaseController', 'sap/ui/model/json/JSONModel'], function(
    BaseController,
    JSONModel
) {
    'use strict';
    return BaseController.extend('<%= projectNamespace %>.controller.Statistics', {
        onInit: function() {
            ...
            try {
                sap.ui.require(
                    ...
                    function() {
                        this.byId('statisticsContainer').addContent(
                            sap.ui.xmlview({
                                id: this.getView().createId('charts'),
                                viewName: '<%= projectNamespace %>.view.StatisticsMicro',
                            })
                        );
                    }.bind(this)
                );
                ...
                this.byId('statisticsContainer').addContent(
                    sap.ui.xmlview({
                        id: this.getView().createId('charts'),
                        viewName: '<%= projectNamespace %>.view.StatisticsD3',
                    })
                );
            }
        },
