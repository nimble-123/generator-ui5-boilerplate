sap.ui.define([
	...
	function getFrameUrl (sHash, sUrlParameters) {
		var sUrl = jQuery.sap.getResourcePath("<%= projectNamespaceAMD %>/app", ".html");
		sHash = sHash || "";
		sUrlParameters = sUrlParameters ? "?" + sUrlParameters : "";

		if (sHash) {
			sHash = "#MyProject-display&/" + (sHash.indexOf("/") === 0 ? sHash.substring(1) : sHash);
		} else {
			sHash = "#MyProject-display";
		}
	...
	return Opa5.extend("<%= projectNamespace %>.test.integration.pages.Common", {
		...
		getMockServer : function () {
			return new Promise(function (success) {
				Opa5.getWindow().sap.ui.require(["<%= projectNamespaceAMD %>/localService/mockserver"], function (mockserver) {
